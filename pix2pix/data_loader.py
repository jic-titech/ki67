...
def load_kmr57_tfdata(
    ...
    for staintype in stains:
        ...
        if (
            staintype == "HE"
            or staintype == "IHC"
            or staintype == "Tensor/IHCpng"
            or staintype == "Tensor/HEpng"
        ):
            ...
            labeled_ds_tr = list_ds_tr.map(
                lambda item: tf.numpy_function(parse_tensor, [item], tf.float32),
                num_parallel_calls=AUTOTUNE,
            )
            labeled_ds_val = list_ds_val.map(
                lambda item: tf.numpy_function(parse_tensor, [item], tf.float32),
                num_parallel_calls=AUTOTUNE,
            )
        elif staintype == "Mask":
    ...
    validation_generator = zip(
        data_generator_val[stains[0]], data_generator_val[stains[1]]
    )
    return (
        (train_generator, len(list_ds_tr)),
        (validation_generator, len(list_ds_val)),
    )
...
def load_kmr_test(
    ...
    data_generator = {}
    for staintype in stains:
        dir_pattern = [dataset_path + "/" + staintype + "/" + "*"]

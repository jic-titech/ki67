...
H_HE_inv = np.linalg.inv(norm_by_row(H_HE))

H_ki67 = H_DAB.copy()
H_ki67[1, :] = np.cross(H_DAB[0, :], H_DAB[2, :])
H_ki67_inv = np.linalg.inv(norm_by_row(H_ki67))
...
def single_prediction(im_in, label, nuclei, net, li_mask=None, net_sizein=256):
    ...
    for i in range(w_num):
        for j in range(h_num):
            ...
            hema_texture = rgbdeconv(chip, H_HE_inv, C=0)[:, :, 0]
...
def interactive_prediction(im_in, net, net_sizein=256, e2e=False):
    ...
    if e2e:
        ...
        hema_texture = rgbdeconv(im_in[S:W-S, S:H-S, :], H_HE_inv, C=0)[:, :, 0]
...
def e2e_pred(im_in, net, sep=False):
    W, H = im_in.shape[0], im_in.shape[1]
    S = (W-2048)//2
    bchip =  np.zeros((1, 2048, 2048, 3))
    bchip[0,:,:,:] = im_in[S:S+2048,S:S+2048,:]
    mask = net.predict(bchip)[0,:,:,:]
    # mask = np.dstack((mask, mask[:, :, 0]))
    if sep:
        # mask = hecconv(mask, H_ki67)
        mask[mask>1.0] = 1.0
        mask[mask<0.0] = 0.0
    res = mask
    hema_texture = mask
...
def e2e_odpred(im_in, net, sep=False):
    W, H = im_in.shape[0], im_in.shape[1]
    S = (W-2048)//2
    bchip =  np.zeros((1, 2048, 2048, 3))
    bchip[0,:,:,:] = im_in[S:S+2048,S:S+2048,:]
    res = net.predict(bchip)[0,:,:,:]
    res = np.dstack((res[:,:,2], res[:,:,1], res[:,:,0]))
    res = hecconv(res, H_ki67)
    mask = res
    hema_texture = mask
    return res, hema_texture, mask